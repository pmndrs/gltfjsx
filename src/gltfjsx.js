...
function codegen({ gltf, filePath }) {
  const objects = []
  ...
  return /* jsx */ `
    ...
    */
    ...
    ${hasAnimations || options.types ? `import * as THREE from 'three'` : ''}
    import React, { useRef ${hasAnimations ? ', useState, useEffect' : ''} } from 'react'
    ${hasAnimations ? `import { useFrame } from 'react-three-fiber'` : ''}
    import { useGLTF } from '@react-three/drei/useGLTF'

    ${
      scene.includes('PerspectiveCamera')
    ...
    export default function Model(props${options.types ? ": JSX.IntrinsicElements['group']" : ''}) {
      ...
      const { 
        ...
        '/${filePath}' ${options.draco ? `, ${JSON.stringify(options.draco)}` : ''}
        ...
        try {
          gltfLoader.parse(
            ...
            (gltf) => {
              ...
              const result = codegen({ gltf, filePath })
